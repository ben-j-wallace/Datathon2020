...
The p-values for `retail_avg`, `grocery_avg`, `workplaces_avg`, and `retail_avg:grocery_avg` are all less than a significance level of $\alpha = 0.05$ and have 95% confidence intervals that do not include 0. Therefore, we can conclude that they are significant predictors when predicting the median property value of owner-occupied homes in thousands of dollars for towns located in the suburbs of Boston. In addition, the interaction term between the mean-centered proportion of non-retail business acres per town and the tract house bounds river is also significant based on the p-value and confidence interval.
...
The estimated coefficient for `indus_cent` is -0.358. This means that for every one percentage point increase in the proportion of non-retail business acres per town, the mean of the median property value of owner-occupied homes for each town is expected to decrease by 358 dollars, holding all other predictor variables constant.

The estimated coefficient for `CHAS1` is 7.485. This means that if a town bounds the Charles River, then the median property value of owner-occupied homes in thousands of dollars for each town is expected to be 7485 dollars higher than the town that does not bound the Charles River, holding all other predictor variables constant.

The estimated coefficient for `rm_cent` is 5.788. This means that for every additional room in the average number of rooms per dwelling, the mean of the median property value of owner-occupied homes for each town is expected to increase by 5788 dollars, holding all other predictor variables constant.

The estimated coefficient for `AGE` is -0.097. This means that for every one percentage point increase in the proportion of owner-occupied units built prior to 1940, the mean of the median property value of owner-occupied homes for each town is expected to decrease by 97 dollars, holding all other predictor variables constant.

The estimated coefficient for `dis_cent` is -1.778. This means that for every one additional mile in the weighted distances to five Boston employment centers, the mean of the median property value of owner-occupied homes for each town is expected to decrease by 1778 dollars, holding all other predictor variables constant.

The estimated coefficient for `ptratio_cent` is -0.599. This means that for every one percentage point increase in pupil-teacher ratio, the mean of the median property value of owner-occupied homes for each town is expected to decrease by 599 dollars, holding all other predictor variables constant.
...
Our model predicts these counties to have the top unemployment rate increases: New York County (+8.7%), Arlington County (+6.9%), Santa Clara County (+6.8%), Kauai County (+6.5%), Maui County (+6.5%).

Our model predicts these counties to have the smallest unemployment rate decreases: Cape May County (-3.3%), Dare County (-3.1%), Door County (-2.5%), Le Sueur County (-0.9%), Taney County (-0.9%).
